...
def download_zip(request):
    try:
        ...
        def optimize_positions(state):
            ...
            for comp_id, pos in positions.items():
                ...
                components.append({
                    ...
                    'x': pos['x'],
                    'y': pos['y'],
                    'width': pos.get('width', 700),
                    'height': pos.get('height', 800 if is_table else 400),
                    'is_table': is_table
                })
            ...
            for row in rows:
                ...
                for comp in row:
                    ...
                    current_x += comp['width'] + 20
                current_y += max_height + 20
            return optimized
        ...
        with zipfile.ZipFile(buffer, 'w') as zip_file:
            ...
            zip_file.writestr('dashboard.json', json.dumps({
                'id': dashboard.id,
                'name': dashboard.name,
                'state': state,
        ...
        response['Content-Disposition'] = f'attachment; filename=dashboard_{dashboard.id}.zip'
        return response
